...
WGS84_OPTIONS = {"a": WGS84.a, "f": WGS84.f, "degrees": True}
SPHERE_OPTIONS = {"a": SPHERE.a, "f": SPHERE.f, "degrees": True}
PROLATE15_OPTIONS = {"a": PROLATE15.a, "f": PROLATE15.f, "degrees": True}
PROLATE30_OPTIONS = {"a": PROLATE30.a, "f": PROLATE30.f, "degrees": True}
...
def test_wgs84_geodesic_distance(testcase):
    ...
    assert s_ab == approx(s12, rel=1e-13)
    assert az_a == approx(azi1, abs=2e-13)
    assert az_b == approx(azi2, abs=1e-13)


@pytest.mark.parametrize("testcase", WGS84_TESTCASES)
def test_wgs84_geodesic_reckon(testcase):
    """Test direct method on the WGS84 ellipsoid"""

    (lat1, lon1, azi1, lat2, lon2, azi2, s12) = testcase[:7]

    lat_b, lon_b, az_b = reckon(lat1, lon1, s12, azi1, long_unroll=True, **WGS84_OPTIONS)

    assert lat_b == approx(lat2, abs=1e-13)
    assert lon_b == approx(lon2, abs=1e-12)
    assert az_b == approx(azi2, abs=1e-13)


@pytest.mark.parametrize("testcase", PROLATE15_TESTCASES)
def test_prolate15_geodesic_reckon(testcase):
    """Test direct method on the prolate15 ellipsoid"""
    (lat1, lon1, azi1, lat2, lon2, azi2, s12) = testcase[:7]

    lat_b, lon_b, az_b = reckon(lat1, lon1, s12, azi1, long_unroll=True, **PROLATE15_OPTIONS)
...
def test_prolate30_direct(testcase):
    ...
    (lat1, lon1, azi1, lat2, lon2, azi2, s12) = testcase[:7]
    lat_b, lon_b, az_b = reckon(lat1, lon1, s12, azi1, long_unroll=True, **PROLATE30_OPTIONS)
    assert lat_b == approx(lat2, abs=1e-13)
...
def test_geo_solve2(lat1, lon1, lat2, lon2, s12, az1, az2):
    ...
    s_ab, az_a, az_b = distance(lat1, lon1, lat2, lon2, **PROLATE15_OPTIONS)
...
def test_geo_solve5():
    ...
    lat_b, lon_b, az_b = reckon(lat1, lon1, 1.0e7, az1, long_unroll=False, **WGS84_OPTIONS)
...
def test_geo_solve12():
    ...
    prolate_geod = {"a": 89.8, "f": -1.83, "degrees": True}
    lat1, lon1, lat2, lon2 = (0, 0, -10, 160)
    s_ab, az_a, az_b = distance(lat1, lon1, lat2, lon2, **prolate_geod)
...
def test_wgs84direct_long_unroll():
    """Check WGS84.direct with and without long unroll"""
    lat_b, lon_b, az_b = reckon(40, -75, 2e7, -10, long_unroll=True, **WGS84_OPTIONS)
...
def test_geo_solve33(datum, lat1, lon1, lat2, lon2, s12, az1, az2):
    ...
    s_ab, az_a, az_b = distance(lat1, lon1, lat2, lon2, **datum)
